...
class SaleOrder(models.Model):
    ...
    def create(self, vals):
        # If bulk create (list of dicts)
        if isinstance(vals, list):
            new_records = []
            for v in vals:
                if v.get('partner_id'):
                    partner = self.env['res.partner'].browse(v['partner_id'])
                    if not v.get('pricing_type'):
                        v['pricing_type'] = partner.pricing_type or 'quantity'
                    if not v.get('customer_type_id'):
                        v['customer_type_id'] = partner.customer_type_id.id or False
                new_records.append(super(SaleOrder, self).create(v))

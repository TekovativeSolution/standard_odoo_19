...
class ProductTemplate(models.Model):
    ...
    #     for rec in self:
    #         rec.is_pricelist_user = (
    #                 self.env.user.has_group("pricelist_extended_tek_17.group_pricelist_user")
    #                 and not self.env.user.has_group("pricelist_extended_tek_17.group_admin_pricelist_user")
    #         )
    ...
    mrp_price = fields.Float("List Price")
    ...
    def _compute_amount(self):
        for rec in self:
            ...
            if base_price:
                ...
                rec.amount = float_round(price, precision_digits=2 ,rounding_method='HALF-UP')  # ✅ ensure 2-decimal consistency
            else:
                rec.amount = 0.0
    ...
    def _compute_margin(self):
        for rec in self:
            ...
            if base_price:
                ...
                rec.margin = float_round(price, precision_digits=2 , rounding_method='HALF-UP')  # ✅ ensure 2-decimal consistency
            else:
                rec.margin = 0.0
...
class ProductLpPurchase(models.Model):
    ...
    def _compute_margin(self):
        for rec in self:
            ...
            if base_price:
                price = abs(rec.amount - base_price)
                rec.margin = float_round(price, precision_digits=2,rounding_method='HALF-UP')
            else:
                rec.margin = 0.0
